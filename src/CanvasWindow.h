...
public:
  ...
  Rectangle drawArea() const {
    return {rect.x, rect.y + (float)TITLE_BAR_HEIGHT, rect.width,
            rect.height - (float)TITLE_BAR_HEIGHT};
  }

  void drawWindow() const;
  void drawPixels(const std::vector<std::vector<Color>> &vpixels,
                  int gridHeight, int gridWidth) const;
  void drawGridLines(int gridHeight, int gridWidth) const;
  void clearCanva(int GRID_HEIGHT, int GRID_WIDTH,
                  std::vector<std::vector<Color>> &vPixels);
