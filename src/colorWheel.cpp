 ...
 *
 *   Example originally created with raylib 6.0, last time updated with
 * raylib 6.0
 *
 *   Example contributed by Robin (@RobinsAviary) and reviewed by Ramon
 * Santamaria (@raysan5)
 *
...
int main(void) {
  ...
  // Indicates if the current color going to be updated, as well as the handle
  // position
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    triangleCount = (unsigned int)Clamp((float)triangleCount, (float)pointsMin,
                                        (float)pointsMax);
    ...
    if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyDown(KEY_C)) {
      if (IsKeyPressed(KEY_C)) {
        SetClipboardText(
            TextFormat("#%02X%02X%02X", color.r, color.g, color.b));
      }
    }
    ...
    if (IsKeyDown(KEY_UP)) {
      ...
      if (pointScale > (float)screenHeight / 2.0f) {
        ...
        circlePosition =
            Vector2Add(Vector2Multiply(Vector2Subtract(circlePosition, center),
                                       (Vector2){1.025f, 1.025f}),
                       center);
      }
    }
    ...
    if (IsKeyDown(KEY_DOWN)) {
      ...
      if (pointScale < 32.0f) {
        ...
        circlePosition =
            Vector2Add(Vector2Multiply(Vector2Subtract(circlePosition, center),
                                       (Vector2){0.975f, 0.975f}),
                       center);
      }

      float distance = Vector2Distance(center, circlePosition) / pointScale;
      float angle = ((Vector2Angle((Vector2){0.0f, -pointScale},
                                   Vector2Subtract(center, circlePosition)) /
                          PI +
                      1.0f) /
                     2.0f);
      ...
      if (distance > 1.0f) {
        circlePosition =
            Vector2Add((Vector2){sinf(angle * (PI * 2.0f)) * pointScale,
                                 -cosf(angle * (PI * 2.0f)) * pointScale},
                       center);
      }
    }
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
        Vector2Distance(GetMousePosition(), center) <= pointScale + 10.0f) {
      settingColor = true;
    }
    ...
    if (settingColor || sliderClicked) {
      ...
      color =
          ColorLerp((Color){(unsigned char)(value * 255.0f),
                            (unsigned char)(value * 255.0f),
                            (unsigned char)(value * 255.0f), 255},
                    ColorFromHSV(angle360, Clamp(distance, 0.0f, 1.0f), 1.0f),
                    valueActual);
    }
    //----------------------------------------------------------------------------------
    ...
    for (unsigned int i = 0; i < triangleCount; i++) {
      ...
      Vector2 offset =
          Vector2Multiply((Vector2){sinf(angle), -cosf(angle)}, scale);
      Vector2 offset2 = Vector2Multiply(
          (Vector2){sinf(angleOffsetCalculated), -cosf(angleOffsetCalculated)},
          scale);
    ...
    if (Vector2Distance(center, circlePosition) / pointScale <= 0.5f &&
        value <= 0.5f) {
    ...
    DrawRectangleLinesEx((Rectangle){8.0f, 8.0f, 64.0f, 64.0f}, 2.0f,
                         ColorLerp(color, BLACK, 0.5f));

    // Draw current color as hex and decimal
    DrawText(TextFormat("#%02X%02X%02X\n(%d, %d, %d)", color.r, color.g,
    ...
    DrawText(TextFormat("triangle count: %d", triangleCount), 8, 395, 20,
             DARKGRAY);
