...
int main() {
  ...
  clsColorButton colorBtns[5] = {// xPos, yPos, 20x20px,  color,  isHovered?
                                 {{230, 10, 20, 20}, BLACK},
                                 {{255, 10, 20, 20}, WHITE},
                                 {{280, 10, 20, 20}, RED},
                                 {{305, 10, 20, 20}, GREEN},
  ...
  while (WindowShouldClose() == false) {
    ...
    for (auto &colorbutton : colorBtns)
      colorbutton.update(mousePos);

    if (createNewBtn.clicked())
      canvas.open(); // convert isOpen to true
    if (clearBtn.clicked() &&
        canvas.openState()) // checks if clearbtn is clicked and if the canva is
                            // open
    {
